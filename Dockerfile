...
RUN apt-get update
RUN apt-get install --no-install-recommends -y wget vim supervisor curl
...
RUN \
    ...
    if ! grep "logstash" /etc/apt/sources.list; then echo "deb http://packages.elasticsearch.org/logstash/2.1/debian stable main" >> /etc/apt/sources.list;fi && \
    apt-get update
